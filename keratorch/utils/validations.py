    ...
    def __setattr__(self, name: str, value: Any, from_update: bool = False):
        ...
        for field_info in fields(self):
            ...
            if field_info.name == name:
                ...
                if hasattr(ftype, '__origin__') and ftype.__origin__ is tuple:
                    if not isinstance(value, tuple):
                        raise TypeError(
                            f"'{self.__class__.__name__}.{fname}' must be a tuple, "
                            f"but got {type(value).__name__}."
                        )
                    
                    elif ftype.__args__ and not all(isinstance(v, ftype.__args__[0]) or v is None for v in value):
                        raise TypeError(
                            f"'{self.__class__.__name__}.{fname}' must be a tuple of {ftype.__args__[0].__name__}, "
                            f"but got a tuple with elements of type {[type(v).__name__ for v in value]}."
                        )
                    
                    elif len(value) != 2:
                        raise ValueError(
                            f"The 'value' must be a tuple of two elements (inputs, targets), but got {len(value)} instead."
                        )
